...
assert.match(migration, /on conflict \(business_id, calendar_identity, start_time, end_time\)[\s\S]*reservation\.operation_id = excluded\.operation_id[\s\S]*reservation\.expires_at <= now\(\)/);
