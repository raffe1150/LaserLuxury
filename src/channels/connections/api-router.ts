...
import { decryptCredential, encryptCredential, credentialKeyId } from './credential-crypto';
...
export async function handleTelegramConnectionUpdate(
  client: SupabaseClient,
  update: any,
): Promise<{ handled: boolean; connectionBusinessId?: number; token?: string; businessConnectionId?: string; translatedUpdate?: any }> {
  ...
  if (update?.business_message?.business_connection_id) {
    ...
    const credential = decryptCredential(data.credential_ciphertext);
    return {
      handled: false,
      connectionBusinessId: Number(data.business_id),
